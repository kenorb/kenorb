...
# Specify web settings
LOGIN_URI="dologin.action" # v5.1.5, older version could have just: login
USER_FIELD="os_username" # v5.1.5, older version could have just username
PASS_FIELD="os_password" # v5.1.5, older version could have just password
...
if [ ! -f "cookies.txt" ]; then 
  wget -O- --keep-session-cookies --save-cookies $COOKIE -U $BROWSER --post-data "$USER_FIELD=$USER&$PASS_FIELD=$PASS" $URL/$LOGIN_URI | grep -q "Log Out" \
